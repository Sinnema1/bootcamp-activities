...
class DeluxeRoom(Room):
    ...
    def __init__(self, room_number):
        ...
        Args:
            room_number (int): The unique number assigned to the room.
        """
        super().__init__("Deluxe", 150, ["Free WiFi", "TV", "Mini Bar", "Ocean View"])
        self.room_number = room_number
...
class Hotel:
    ...
    def make_reservation(self, room_number):
        ...
        if room_number in [room.room_number for room in self.available_rooms]:
            self.reservations.append(room_number)
            
            self.available_rooms = [room for room in self.available_rooms if room.room_number != room_number]
        else:
    ...
    def cancel_reservation(self, room_number):
        for room in self.reservations:
            if room.room_number == room_number:
                ...
                print(f"Room {room_number} reservation has been canceled.")
                return

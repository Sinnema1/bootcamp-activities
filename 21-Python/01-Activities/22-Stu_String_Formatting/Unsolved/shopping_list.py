...
def show_list():
    ...
    if not shopping_list:
        ...
        for item_name, item in shopping_list.items():
            if "price" in item:
                # gets the price of an item with 2 decimal places
                price = f'${item["price"]:,.2f}'
            else:
                price = "unknown"

            # Append a formatted string of the item to the formatted_items list
            formatted_items.append(f"{item_name}: {item['quantity']} @ "
                                f"{price} each")

            # Check if the length of the last formatted item is greater than the current max_print_length
            # If so, update max_print_length to the length of the last formatted item
            if len(formatted_items[-1]) > max_print_length:
                max_print_length = len(formatted_items[-1])

    # Create a border string with a length of max_print_length + 4
    border_string = "-" * (max_print_length + 4)

    # Print the border string
    print(border_string)
    # Print the title "Shopping List:" centered within the border
    print("|" + "Shopping List:".center(max_print_length + 2) + "|")
    # Print the border string again
    print(border_string)

    for item in formatted_items:
        if len(item) < max_print_length:
            # Print each item left-justified within the border
            print(f"| {item.ljust(max_print_length)} |")
        else:
            # Print each item as is within the border
            print(f"| {item} |")
    # Print the border string again
    print(border_string)

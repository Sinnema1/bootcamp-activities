...
function InvoiceForm({ addItem }: Props) {
  ...
  const handleSubmit = (e: React.SyntheticEvent) => {
    ...
    // ensure data collected from user matches the shape of a Charge object.
    const chargeData = {
      billed: formData.charge.billed,
      price: parseFloat(formData.charge.rate) || 0,
    } as Charge;
    //TODO: What purpose does this `if` statement serve?
    // if hourly, the optional keys are added and update charge to reflect cost
    ...
    // funtion passed in from a prop to add charge to invoice
